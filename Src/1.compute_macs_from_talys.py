...
sup_rev = str.maketrans("⁰¹²³⁴⁵⁶⁷⁸⁹", "0123456789")
...
def compute_macs_for_files(talys_files, kT_list_keV, out_csv_path):
    ...
    for fpath in talys_files:
        ...
        if not m:
            raise ValueError(f"Could not parse isotope from filename: {fpath}")

        A_str = m.group(1)           
        Z = m.group(2)              

        
        A = int(A_str.translate(sup_rev))
...
if __name__ == "__main__":
    ...
    kT_list = [5,10,15,20,25,30,40,50,60,80,100]
